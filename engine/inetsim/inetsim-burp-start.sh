...
function main () {
	...
	# HTTPS
	iptables -t nat -A PREROUTING -i enp0s8 -p tcp --dport 443 -j REDIRECT --to-port 8443

	# SMTP
	iptables -t nat -A PREROUTING -i enp0s8 -p tcp --dport 465 -j REDIRECT --to-port 8465
